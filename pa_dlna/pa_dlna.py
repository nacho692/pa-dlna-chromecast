...
class PulseEvent:
    ...
    def __init__(self, event, index, prev_state, new_state):
        self.event = event
        self.index = index
    ...
    def __eq__(self, other):
        return (self.event == other.event and
                ...
                self.prev_state == other.prev_state and
...
class Renderer:
    ...
    async def handle_pulse_event(self):
        ...
        # Do not log a Pulseaudio event identical to the previous one.
        index = None if sink_input is None else sink_input.index
        pulse_event = PulseEvent(event, index, prev_state, new_state)
        log_event = (self.pulse_event is None or
